<?php 
    ...
    if(isset($_GET['id'])){
        $id = $_GET['id'];
        $sql_1 =  "SELECT * from experience where id_experience = {$id}";
        ...
        if(isset($_POST['txtSubmit'])){
            ...
            $time = $_POST['txttime'];
            $dess = $_POST['txtdess'];
            $sql = "UPDATE experience SET name='$name', time='$time',desscription='$dess' WHERE id_experience ='$id' "; 

            if(mysqli_query($db, $sql)){
                // echo "Inserted successfully ^^";
                header("Location: http://localhost/test_api/admin/experience/viewExpen.php");   //cai này là link sau khi cái if = true thì nó sẽ chạy về viewDoing
                // echo "$img";
            }
            else{
                echo "Error!";
